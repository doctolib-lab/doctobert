"""
Finetune a classification or regression model.
...
class DataTrainingArguments:
    ...
    do_regression: bool = field(
        default=None,
        metadata={"help": "Whether to do regression instead of classification. If None, will be inferred from the dataset."},
    )
    ...
    url_column_name: Optional[str] = field(
        default="url",
        metadata={"help": "The name of the url column in the input dataset."},
    )
    ...
    max_predict_samples: Optional[int] = field(
        ...
        metadata={
            "help": (
                "For debugging purposes or quicker training, truncate the number of prediction examples to this value if set."
            )
        },
    )
...
def maybe_load_multiple_local_datasets(dataset_paths: str) -> Dataset:
    """Load datasets from paths and concatenate them."""
    datasets = []
    for dataset_path in dataset_paths.split("+"):
        ds = load_dataset(dataset_path, split="train")
        print(f"Loaded {ds.num_rows:,d} examples from {dataset_path}")
        datasets.append(ds)
    ...
    print(f"Concatenated into {final_dataset.num_rows:,d} examples")
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        # Loading a dataset from local dir
        raw_datasets = DatasetDict()
        raw_datasets["train"] = maybe_load_multiple_local_datasets(data_args.train_file)
        if data_args.validation_file is not None:
            raw_datasets["validation"] = maybe_load_multiple_local_datasets(data_args.validation_file)
        if data_args.test_file is not None:
            raw_datasets["test"] = maybe_load_multiple_local_datasets(data_args.test_file)
    ...
    def preprocess_function(examples):
        # Build sentences in batched mode (column-wise), optionally prefix with URL
        if data_args.text_column_names is not None:
            text_column_names = data_args.text_column_names.split(",")
            num_examples = len(examples[text_column_names[0]])
            sentences = []
            for i in range(num_examples):
                parts = [examples[col][i] for col in text_column_names]
                sentence = data_args.text_column_delimiter.join(parts)
                if data_args.url_column_name in examples:
                    sentence = f"{examples[data_args.url_column_name][i]}\n\n{sentence}"
                sentences.append(sentence)
        else:
            # Expect a pre-computed "sentence" column
            sentences = examples["sentence"]
            if data_args.url_column_name in examples:
                sentences = [f"{examples[data_args.url_column_name][i]}\n\n{sentences[i]}" for i in range(len(sentences))]
    ...
    if training_args.do_predict:
        ...
        if is_regression:
            ...
            # round up quality score and compute classification report
            cls_report = classification_report(labels, np.round(predictions))
            logger.info(cls_report)
            output_predict_file = os.path.join(training_args.output_dir, "predict_results.txt")
            with open(output_predict_file, "w") as writer:
                writer.write(cls_report)
            logger.info(f"Classification report saved at {output_predict_file}")
            # # floor quality score and compute classification report
            # cls_report = classification_report(labels, np.floor(predictions))
            # logger.info(cls_report)
            # output_predict_file = os.path.join(training_args.output_dir, "predict_results_floor.txt")
            # with open(output_predict_file, "w") as writer:
            #     writer.write(cls_report)
            # logger.info(f"Classification report saved at {output_predict_file}")
            # # ceil quality score and compute classification report
            # cls_report = classification_report(labels, np.ceil(predictions))
            # logger.info(cls_report)
            # output_predict_file = os.path.join(training_args.output_dir, "predict_results_ceil.txt")
            # with open(output_predict_file, "w") as writer:
            #     writer.write(cls_report)
            # logger.info(f"Classification report saved at {output_predict_file}")
        elif is_multi_label:

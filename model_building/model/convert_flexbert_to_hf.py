# https://github.com/AnswerDotAI/ModernBERT/issues/186
...
def update_config(
    ...
    target_config = {
        ...
        # use same activation for cls and mlm
        "classifier_activation": source_config.get("head_pred_act", source_config["hidden_act"]),
        "classifier_bias": source_config["head_class_bias"],
        # "classifier_bias": True, # source_config["head_class_bias"],  # todo
        "classifier_dropout": source_config["head_class_dropout"],
        "classifier_pooling": "mean",
        # "classifier_pooling": "cls", # "mean",  # todo
...
def main(
    ...
    model_type: Annotated[ModelType, Option(help="Model type: flexbert (default) or roberta.")] = ModelType.flexbert,
    ...
    safetensors: Annotated[bool, Option(help="Save weights as a model.skeafetensors file.")] = True,
    ...
    if model_type == ModelType.roberta:
        # Only update tokenizer_config.json for roberta models
        ...
        # Align tokenizer.pad_token_id with model.config.pad_token_id so HF Trainer's
        # auto-sync (`model.config.pad_token_id ← tokenizer.pad_token_id`) is a no-op.
        # The Composer-trained RoBERTa has model.config.pad_token_id=1 (HF roberta-base
        # default), but our [PAD] token is at id 32682 in vocab_32680. The mismatch
        # breaks fine-tune→save→reload because the new RobertaEmbeddings asserts
        # `pad_token_id < max_position_embeddings` (32682 ≥ 512 → crash). Fix: repurpose
        # id 1 as the pad sentinel. Real text never emits id 1 (it's a normalized
        # single-space token), so it's safe to use for padding. See docs/ for context.
        from transformers import AutoTokenizer
        config = json.load(open(f"{target_path}/config.json"))
        model_pad_token_id = config.get("pad_token_id", 1)
        tok = AutoTokenizer.from_pretrained(target_path)
        if tok.pad_token_id != model_pad_token_id:
            tok.pad_token = tok.convert_ids_to_tokens(model_pad_token_id)
            tok.save_pretrained(target_path)
        return

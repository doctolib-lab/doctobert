...
def main(
    ...
    # tmp: rename pre-existing prediction columns from a prior run so the new
    # run can write fresh `{task_name}_*` columns without collision
    for col in (
        f"{task_name}_score",
        f"{task_name}_normalized_score",
        f"{task_name}_scores",
        f"{task_name}_best_class",
        f"{task_name}_best_score",
    ):
        if col in dataset.column_names:
            dataset = dataset.rename_column(col, f"original_{col}")
            print(f"Renamed pre-existing column {col} -> original_{col}")
    ...
    if "nvidia/multilingual-domain-classifier" in model_name_or_path:
        ...
        model = AutoModelForSequenceClassification.from_pretrained(model_name_or_path, torch_dtype=torch.bfloat16)

...
try:
    ...
    if parse(installed_version) < parse("2.6.4"):
        raise ValueError("newer version of flash_attn required (>= 2.6.4) for flash attention 2")
...
class FlexBertAttention(FlexBertAttentionBase):
    ...
    def __init__(self, config: FlexBertConfig, layer_id: Optional[int] = None):
        ...
        if config.use_fa and not self.use_fa2 and not self.use_fa3:
            ...
            self.use_fa2 = False
            self.use_fa3 = False
        if not self.use_fa2 and not self.use_fa3:
    ...
    def forward(
        ...
        if self.use_fa3:
            ...
            if convert_dtype:
                orig_dtype = qkv.dtype
                qkv = qkv.to(torch.bfloat16)
                # Unbind q, k, v separately for FA3
                ...
                attn, _ = flash_attn_varlen_func(
                    ...
                    seqused_q=used_seqlens,
                    seqused_k=used_seqlens,
...
ATTN2CLS = {
    "base": FlexBertAttention,
    "parallel": FlexBertParallelAttention,
    "rope": FlexBertRopeAttention,
    "rope_parallel": FlexBertRopeParallelAttention,
}

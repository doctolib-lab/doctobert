...
from src.top_accuracy import MaskedTop1Accuracy, MaskedTop5Accuracy, MaskedTop10Accuracy, MaskedTop25Accuracy
import os

def create_modernbert_mlm(
    pretrained_model_name: str = "modernBERT-base",
    ...
    if use_pretrained:
        assert transformers.ModernBertForMaskedLM.from_pretrained is not None, (
            "AutoModelForMaskedLM has from_pretrained method"
        )
        config = transformers.ModernBertConfig.from_pretrained(
            pretrained_model_name, **model_config
        )
        model = transformers.ModernBertForMaskedLM.from_pretrained(
            pretrained_model_name_or_path=pretrained_model_name, config=config,
        )
    else:
        config = transformers.ModernBertConfig.from_pretrained(
            pretrained_model_name, **model_config
        )
        assert transformers.ModernBertForMaskedLM.from_config is not None, (
            "AutoModelForMaskedLM has from_config method"
        )
        model = transformers.ModernBertForMaskedLM.from_config(config)
    ...
    metrics = [
        ...
        MaskedTop1Accuracy(ignore_index=-100),
        MaskedTop5Accuracy(ignore_index=-100),
        MaskedTop10Accuracy(ignore_index=-100),
        MaskedTop25Accuracy(ignore_index=-100)
    ]
    ...
    if padding_needed != 0:
        ...
        tokenizer.add_tokens(dummy_tokens, special_tokens=False)
    hf_model.model.resize_token_embeddings(len(tokenizer))
